...
app.post("/collect/allproduct", async (req, res) => {
  //console.log(req)
  try {
    res.setHeader('Access-Control-Allow-Origin', '*')
    const returndata = await dbcaller.CollectingData()
    // console.log("posr",returndata)
...
app.post("/collect/product/:productid", async (req, res) => {
  try {
    res.setHeader('Access-Control-Allow-Origin', '*')
    const productid = req.params.productid
    const returndata = await dbcaller.CollProductinfo(productid)
    // console.log(returndata)
    if (returndata == "" || returndata == null || returndata === false) {
      res.status(500).send(returndata)
    }
    else {
      res.status(200).send(returndata)
...
// function baseconver(myfile) {
//   // console.log(myfile)
//   var body = fs.readFileSync(myfile);
//   // body = Buffer.from(body,"base64")
//   let buffobj = body.toString("base64")
//   //console.log(buffobj)

//   return body
//   "data:image/jpg;base64," + fs.readFileSync(myfile, 'base64');
// }
// var checkstring = baseconver('source-images/brand-1.png')
// //createimg(checkstring)
// function createimg(checkstring) {
//   fs.writeFileSync('1.png', checkstring)
// }
